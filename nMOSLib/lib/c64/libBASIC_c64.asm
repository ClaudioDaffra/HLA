...
basic	.proc
	...
	.weak
		...
		_conv_u8_to_fac1		=	$b3a2	;	a	Convert u8  held in Y to a FP number in FAC1
		conv_s8_to_fac1			=	$bc3c	;	a	Convert s8  held in A to a FP number in FAC1
		_conv_s16_to_fac1		=	$b391   ;   ay  Convert s16 held in Y/A (lo/high) to a FP number in FAC1
	...
	store_fac2_inc_fac1	=	$bbfc ; Stores FAC2 in FAC1.
	...
	.weak
		...
		neg_fac1   					= 	$bfb4	;	49076	makes fac1 negative
		neg_fac1_kb					= 	$bfb4	;	49076	makes fac1 negative
		...
		sgn_fac1_kb_a				=	$bc2b 	;	Evaluate sign of FAC1 (returned in A)

		sgn_fac1_kb_fac1			=	$bc39	;	Evaluate sign of FAC1 (returned in FAC1)
	...
	not_fac1	.proc
		lda #<basic.zero
		ldy #>basic.zero
		jsr $bc5b  ; -> a ;;;; compare float accu to float indexed by XY
		; = Compare FAC1 with memory contents at A/Y (lo/high)
		;	$00 FAC1 == AY
		;	$01 FAC1  > AY
		;   $FF FAC1  < AY
		;beq basic.true		;	fac1 = 1.0
		;bne basic.false	;	fac2 = 0.0
		beq one
		bne zero
		rts
	one
