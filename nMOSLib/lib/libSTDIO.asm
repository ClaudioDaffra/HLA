...
std	.proc
	...
	print_uword	.proc
		...
		jmp basic.print_unsigned_integer
   ...
   .endproc
	...
	print_string	.proc
		...
		ldx #$00              ; Inizializza X a 0
	stampa_loop:
		lda $FFFF, x      	  ; Carica in A il carattere all'indice X
		beq fine              ; Se A è zero (terminatore), salta a fine
		jsr kernel.chrout     ; Stampa il carattere contenuto in A
		inx                   ; Incrementa X per il prossimo carattere
		beq fine			  ; check max 255 character
		jmp stampa_loop       ; Ripete il ciclo
	fine:
